...
def STA_LTA(trace, i, ns, nl, threshold):
    ...
    if ratio > threshold:
        ...
        is_detected = False
    return is_detected, i, sta_energy, lta_energy
...
def detection_STA_LTA():
    ...
    while i < len(trace):
        ...
        is_detected, detection_index, sta_energy, lta_energy = STA_LTA(trace, i, ns, nl, threshold)
        ...
        if is_detected:
            ...
            for k in range(len(trace)):
                if detection_index >= k and k >= detection_index - nl:
                    ...
                    lta_list[k] = 0
            print(detection_index, sta_list, lta_list, trace)       
            return detection_index, sta_list, lta_list, trace
        
        i += 1 
    print(-1, sta_list, lta_list, trace)
